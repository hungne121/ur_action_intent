...
class UR5Robotiq85:
    ...
    def _parse_joint_info(self):
        ...
        # Dynamically locate the End-Effector link ID (prefer tool0/flange-tool0)
        eef_candidates = ("tool0", "flange-tool0", "susgrip_base_joint", "ee_fixed_joint", "wrist_3_joint")
        self.eef_id = self.joints[0].id
        for candidate in eef_candidates:
            matched = [j for j in self.joints if j.name == candidate]
            if matched:
                self.eef_id = matched[0].id
                break

        # Split the first six controllable joints as the UR arm joints.
        ...
        # Build full joint limit lists for PyBullet calculateInverseKinematics
        home_dict = {
            self.arm_controllable_joints[0]: 0.0,
            self.arm_controllable_joints[1]: -1.40,
            self.arm_controllable_joints[2]: 1.45,
            self.arm_controllable_joints[3]: -1.60,
            self.arm_controllable_joints[4]: -1.57,
            self.arm_controllable_joints[5]: 0.0,
        }
        ...
        for j in self.joints:
            if j.controllable:
                lower, upper = j.lowerLimit, j.upperLimit
                if lower >= upper:
                ...
                self.ik_rest_poses.append(home_dict.get(j.id, 0.0))

    def __setup_mimic_joints__(self):
        """Tie child gripper joints to parent finger joint."""
        mimic_parents = [joint for joint in self.joints if joint.name in ("finger_joint", "gripper_joint")]
        ...
        self.mimic_child_info = {
            joint.id: mimic_children_info[joint.name]
            for joint in self.joints
            if joint.name in mimic_children_info
        }
        self.mimic_child_multiplier = {
            j_id: info[0] for j_id, info in self.mimic_child_info.items()
        }

        parent_joint = [j for j in self.joints if j.id == self.mimic_parent_id]
        if parent_joint and parent_joint[0].name != "gripper_joint":
            # Create gear constraints for Robotiq 85
    ...
    def _apply_authentic_robot_colors(self):
        """Apply authentic real-world Universal Robots UR3e and SusGrip 2F color palette."""
        ur_silver = [0.82, 0.82, 0.85, 1.0]     # Silver/aluminum main arm links
        ur_blue = [0.0, 0.48, 0.72, 1.0]        # Iconic UR blue joint caps
        ur_dark = [0.22, 0.22, 0.25, 1.0]        # Dark charcoal base & wrist coupling
        grip_black = [0.15, 0.15, 0.17, 1.0]    # Sleek matte black gripper base & sliders
        grip_silver = [0.85, 0.85, 0.88, 1.0]   # Industrial silver/chrome gripper linkages
        grip_pad = [0.10, 0.10, 0.12, 1.0]      # High-grip black rubber pads
        ...
        for i in range(-1, p.getNumJoints(self.id)):
            if i == -1:
                ...
                name = p.getJointInfo(self.id, i)[12].decode()
            joint_map[name] = i

        for link_name, rgba in color_mapping.items():
            if link_name in joint_map:
                p.changeVisualShape(self.id, joint_map[link_name], rgbaColor=rgba)
    ...
    def move_arm_ik_absolute(self, target_pos, target_orn):
        ...
        # Check parent joint name to set correct gripper range
        if hasattr(self, 'mimic_parent_id') and self.mimic_parent_id is not None:
            parent_joint = [j for j in self.joints if j.id == self.mimic_parent_id]
            if parent_joint and parent_joint[0].name == "gripper_joint":
    ...
    def move_gripper(self, open_length):
        ...
        if getattr(self, 'mimic_parent_id', None) is None:
            return
        ...
        if parent_joint and parent_joint[0].name == "gripper_joint":
            ...
            for j_id, (mult, offset) in getattr(self, 'mimic_child_info', {}).items():
                p.setJointMotorControl2(
                    self.id,
                    j_id,
                    p.POSITION_CONTROL,
            ...
            target_pos = 0.715 - math.asin((open_length - 0.010) / 0.1143)
            p.setJointMotorControl2(
                self.id,
                self.mimic_parent_id,
                p.POSITION_CONTROL,
...
class UR5PickCubeEnv:
    ...
    def sample_cube_pose(self):
        ...
        # Randomize the cube within a safe tabletop workspace reachable by UR3e.
        x = self.rng.uniform(0.32, 0.42)
        y = self.rng.uniform(-0.10, 0.10)
        z = 0.65
        yaw = 0.0
    ...
    def reset(self):
        ...
        self.robot.move_gripper(self.robot.gripper_range[1])
        self.step_sim(30)
        ...
        p.changeDynamics(self.cube_id, -1, lateralFriction=3.0, spinningFriction=0.1, frictionAnchor=1)
        for i in range(p.getNumJoints(self.robot.id)):
            p.changeDynamics(self.robot.id, i, lateralFriction=3.0, spinningFriction=0.1, frictionAnchor=1)
    ...
    def gripper_close(self):
        """Close the gripper until fingers contact the cube or fully close."""
        grip_value = self.robot.gripper_range[1]
        max_iters = 15
        step_size = (self.robot.gripper_range[1] - self.robot.gripper_range[0]) / max_iters

        for _ in range(max_iters):
            contact_points = p.getContactPoints(bodyA=self.robot.id, bodyB=self.cube_id)
            if len(contact_points) > 0:
                print(f"[Grasped] contacts={len(contact_points)}")
                return True

            if grip_value <= self.robot.gripper_range[0]:
                break

            grip_value -= step_size
    ...
    def apply_action(self, action):
        ...
        # Apply position delta while preventing end effector from going too low.
        ...
        # Interpret gripper values above threshold as open, otherwise close or hold closed.
        if gripper > 0.02:
            self.robot.move_gripper(self.robot.gripper_range[1])
            self.step_sim(5)
            self.has_closed_gripper = False
...
def make_expert_action(env, phase):
    ...
    # Fixed waypoint heights relative to table level (table z=0.62, cube top z=0.67)
    approach_height = 0.87
    pregrasp_height = 0.83
    grasp_height = 0.805
    lift_height = 0.93

    open_gripper_val = env.robot.gripper_range[1]

    xy_noise = np.random.uniform(-0.002, 0.002, size=2).astype(np.float32)
    z_noise = np.random.uniform(-0.001, 0.001, size=1).astype(np.float32)
...
def phase_done(env, phase, phase_step_count):
    ...
    if phase == "approach_cube":
        target = np.array([cube_pos[0], cube_pos[1], approach_height], dtype=np.float32)
        return np.linalg.norm(ee_pos - target) < 0.03

    if phase == "pre_descend":
        target = np.array([cube_pos[0], cube_pos[1], pregrasp_height], dtype=np.float32)
        return np.linalg.norm(ee_pos - target) < 0.025

    if phase == "descend":
        target = np.array([cube_pos[0], cube_pos[1], grasp_height], dtype=np.float32)
        return np.linalg.norm(ee_pos - target) < 0.018

    if phase == "close":
        return env.has_closed_gripper or phase_step_count >= 8

    if phase == "hold_close":
        return phase_step_count >= 6

    if phase == "lift":
        return env.is_success() or phase_step_count >= 20
